            ...
            <div class="row poll-container">
                <div class="col-12">
                    ...
                    <div class="row poll-options">
                        <div class="col-12">
                            <div class="custom-control custom-checkbox">
                                <input tabindex="1" v-model="poll.multiple" type="checkbox"
                                       class="checked custom-control-input" id="cb-multiple-votes">
                                <label class="custom-control-label" for="cb-multiple-votes">{{
                                    $t('plugins.poll.allow_multiple_votes') }}</label>
                            </div>
                        </div>
                    </div>
                    <div class="row poll-options">
                    ...
                    <div class="row poll-actions">
                        <div class="col-12">
                            <button v-if="editPoll" @click="createPoll" class="btn btn-outline-primary"
                                    :disabled="!validate()">{{ $t('plugins.poll.save') }}
                            </button>
                            <button v-if="!editPoll" @click="createPoll" class="btn btn-outline-primary"
                                    :disabled="!validate()">{{ $t('plugins.poll.create') }}
                            </button>
                            <button @click="closePoll" class="btn btn-outline-secondary">{{ $t('plugins.poll.cancel')
                                }}
                            </button>
                            <button @click="deletePoll" class="btn btn-outline-danger" :disabled="!editPoll">{{
                                $t('plugins.poll.delete') }}
                            </button>
                        </div>
                    </div>
                    <!--
...
<script>
    ...
    export default {
        ...
        methods: {
            addEntry(text) {
                const max = Math.max.apply(Math, this.poll.answers.map(function (o) {
                    return o.value
                }))
