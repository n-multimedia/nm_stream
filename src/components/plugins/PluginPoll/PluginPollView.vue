<template>
    <div class="vue-poll">
        ...
        <div class="votes" v-if="showTotalVotes && (visibleResults || finalResults)" v-text="totalVotesFormatted + ' ' + labelVotes"></div>

        <template v-if="!finalResults && !visibleResults && multiple && totalSelections > 0">
             <button href="#" @click.prevent="handleMultiple" class="btn btn-primary submit-button" v-text="submitButtonText"></button>
        </template>
