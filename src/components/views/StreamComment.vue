              ...
              <div class="row">
                <div class="col-12">
                  ...
                  <div class="created">
                    ...
                    <span v-if="comment.changed != comment.created"
                          :title="comment.changed | moment('L LT')"> ({{ $t('label.edited') }})</span>
                  </div>
                </div>
              </div>
              <div class="row">
...
export default {
  ...
  methods: {
    ...
    updateComment(comment) {
      this.$emit('stream-comment-updated', comment)
    ...
    onDelete() {
      ...
      Vue.axios.get(apiCommentDeleteUrl, {withCredentials: true}).then((response) => {
        if (response.data.status === 1) {
          ...
          this.$store.dispatch('deleteComment', {containerNID: this.streamOptions.containerNID, comment: self.comment})
